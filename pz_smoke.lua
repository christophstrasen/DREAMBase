-- PZ-style loader smoke test for the DREAMBase ecosystem build.
-- Run after syncing to catch missing require/path issues before launching the game.

local function join(...)
	return table.concat({ ... }, ";")
end

-- Default path assumes you're running this from a synced Workshop wrapper.
-- Prefer passing PZ_LUA_PATH from a dev script that points to $HOME/Zomboid/mods.
...
if #arg > 0 then
	...
	modules = { "DREAMBase", "PromiseKeeper", "SceneBuilder", "WorldObserver", "LQR", "reactivex" }
end
...
local function exercise_modules(loaded)
	local DB = loaded.DREAMBase
	if DB then
		assert(type(DB) == "table", "DREAMBase did not return a table")
		assert(DB.util ~= nil, "DREAMBase missing util")
		assert(DB.log ~= nil, "DREAMBase missing log")
	end
	...
	local PromiseKeeper = loaded.PromiseKeeper
	if PromiseKeeper then
		assert(type(PromiseKeeper) == "table", "PromiseKeeper did not return a table")
		assert(type(PromiseKeeper.namespace) == "function", "PromiseKeeper missing namespace()")
	end

	local SceneBuilder = loaded.SceneBuilder
	if SceneBuilder then
		assert(type(SceneBuilder) == "table", "SceneBuilder did not return a table")
	end
